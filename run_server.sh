...
if [[ "$1" == 'test' ]];then
     java -cp ./third_party/sqlite-jdbc-3.16.1.jar:./third_party/opennlp-tools-1.8.0.jar:./third_party/opennlp-uima-1.8.0.jar:./bin codeu.chat.ServerMain 100 'ABABAB' 2222 '.'
else
    TEAM_ID="$1"
    TEAM_SECRET="$2"
    PORT="$3"
    ...
    if [ "$RELAY_ADDRESS" == "" ] ; then
      java -cp ./third_party/sqlite-jdbc-3.16.1.jar:./third_party/opennlp-tools-1.8.0.jar:./third_party/opennlp-uima-1.8.0.jar:./bin codeu.chat.ServerMain \
          "$TEAM_ID" \
          "$TEAM_SECRET" \
          "$PORT" \
          "$PERSISTENT_DIR"
